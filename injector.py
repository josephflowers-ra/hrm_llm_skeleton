...
class InjectorGRB(nn.Module):
    ...
    def __init__(self, d_h: int = 512, d_model: int = 2048):
        ...
        # Learned scalar gate; sigmoid(gate) ∈ (0,1). Start gentle if desired.
        self.gate = nn.Parameter(torch.tensor(-2.0))  # ≈ 0.12 after sigmoid

    def forward(self, last_hidden: torch.Tensor, zH: torch.Tensor) -> torch.Tensor:
        bias = self.proj(zH)  # (B,1,D) broadcasts across T
...
class CrossAttentionBridge(nn.Module):
    ...
    def __init__(self, d_h: int = 512, d_model: int = 2048, n_heads: int = 8):
        ...
        # Learned scalar gate; sigmoid(gate) ∈ (0,1). Start gentle.
        self.gate = nn.Parameter(torch.tensor(-2.0))  # ≈ 0.12 after sigmoid
    ...
    def forward(self, last_hidden: torch.Tensor, zH: torch.Tensor) -> torch.Tensor:
        ...
        # Scaled dot-product attention: logits (B,h,T,1)
        att_logits = (Q @ K.transpose(-1, -2)) / (self.hd ** 0.5)

        # Softmax over KEYS (last dim); with 1 key it's a no-op but correct for >1 later
        att = torch.softmax(att_logits, dim=-1)  # (B,h,T,1)

        # Weighted sum over V → merge heads → project → residual
        out = att @ V                              # (B,h,T,hd)
